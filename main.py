...
def get_parameters():
    ...
    parser.add_argument('--seed', type=int, default=100, help='set the random seed for stabilize experiment results, \
                        it should not be tuned as a hyperparameter')
    parser.add_argument('--dataset_config_path', type=str, default='./config/data/cora.ini', \
                        ...
                        citeseer.ini for CiteSeer, pubmed.ini for PubMed')
    parser.add_argument('--model_config_path', type=str, default='./config/model/ssgc_sym.ini', \
                        help='the path of model config file, \
                        ssgc_sym.ini for SSGC with symmetric renormalized adjacency matrix, \
                        rw means random walk renormalized adjacency matrix')
    parser.add_argument('--alpha', type=float, default=0.05, help='balance hyperparameter, \
                        alpha in [0, 1], default as 0.05 for citation networks')                      
    parser.add_argument('--K', type=int, default=16, help='K power, default as 16 for citation networks')                    
    parser.add_argument('--enable_bias', type=bool, default=True, help='using bias in graph convolution layers or not')
    ...
    if (renorm_adj_type != 'sym') and (renorm_adj_type != 'rw'):
        raise ValueError(f'ERROR: The type of renormalized adjacency matrix is undefined.')

    if mode == 'tuning':
        param = nni.get_next_parameter()
        alpha, K = [*param.values()]
    else:
        if args.alpha > 1 or args.alpha < 0:
            raise ValueError(f'ERROR: alpha should be in [0, 1].')
        else:
            alpha = args.alpha
        if args.K < 2:
            raise ValueError(f'ERROR: K has to be greater than 1.')
        else:
            K = args.K
...
if __name__ == "__main__":
    device, dataset_name, data_path, graph_path, learning_rate, weight_decay_rate, model_name, model_save_path, renorm_adj_type, alpha, K, enable_bias, epochs, opt, early_stopping_patience = get_parameters()
